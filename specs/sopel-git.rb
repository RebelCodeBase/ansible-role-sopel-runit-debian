...
describe "Git config user.name in \"#{configdir}/.git\"" do
  it "should be #{pname}" do
    sopel_git_config_user_name_configdir = command(git_config_user_name).stdout.chomp
    expect(sopel_git_config_user_name_configdir).to eq "#{pname}"
  end
end
...
describe "Git config user.email in \"#{configdir}/.git\"" do
  it "should be #{pname}@#{host}" do
    sopel_git_config_user_email_configdir = command(git_config_user_email).stdout.chomp
    expect(sopel_git_config_user_email_configdir).to eq "#{pname}@#{host}"
  end
end
